...
void draw() {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      ...
      if (isMouseOverShape(x, y)) {
        noFill();
        stroke(alternateColor ? color(255, 0, 229) : color(204, 255, 0)); // Change stroke color when mouse is over the shape
...
boolean isMouseOverShape(float x, float y) {
  return dist(x, y, mouseX, mouseY) < shapeSize / 2;
}
