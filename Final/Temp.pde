...
int roundCount = 1; // Track the number of rounds completed
int displayTime = 15000; // 15 seconds display time

void setup() {
  size(500, 500);
  cols = floor(width / (2 * bubbleRadius));
  rows = floor(height / (2 * bubbleRadius));
  spacing = width / cols + 0.6;
...
void draw() {
  ...
  if (allBubblesBurst()) {
    ...
    if (millis() - sprinkleStartTime > displayTime) {
      initializeBubbleWrap(); // Reset bubbles
      roundCount++; // Increment the round count
      sprinkleStartTime = -1; // Reset the timer
    }
  }
...
void displayLevelCompletedMessage() {
  fill(148, 87, 235); // Set text color to unicorn color (purple-pink)
  
  // Adjust text size based on window width
  float textSizeFactor = width / 800.0; // Adjust this factor based on your preference
...
class Sprinkle {
  ...
  Sprinkle(float x, float y, float angle, color bubbleColor) {
    ...
    this.y = y;
    this.angle = angle;
    this.speedX = cos(angle) * 5; // Adjust the speed as needed
    this.speedY = sin(angle) * 5; // Adjust the speed as needed
    this.length = 3.0;
